...
class ClassmatesListScreen extends StatefulWidget {
  const ClassmatesListScreen({Key? key}) : super(key: key);

  @override
  State<ClassmatesListScreen> createState() => _ClassmatesListScreenState();
}

class _ClassmatesListScreenState extends State<ClassmatesListScreen> {
  String studentClass = '';
  ...
  getData() {
    firestore
      ...
      studentClass = snapshot.get('class');
      print(studentClass);
      studentSection = snapshot.get('section');
      print(studentClass);
      setState(() {});
    });
  }
  ...
  void getSuperData() async {
    docsLength = await initalize();
  }

  Future<String> initalize() async {
    String length = await firestore
        .collection('classes')
        .doc('${studentClass}th')
        .collection('${studentClass}-${studentSection}')
        .doc('students')
        .collection('students')
        .snapshots()
        .length
